...
class Crew(FlowTrackable, BaseModel):
    ...
    def kickoff(
        ...
        try:
            # 准备kickoff，当有before_kickoff_callbacks时，会先执行before_kickoff_callbacks
            inputs = prepare_kickoff(self, inputs, input_files)

            # 根据process模式执行，sequential模式下，会依次执行每个任务，hierarchical模式下，会先执行管理员代理，然后执行每个任务
            if self.process == Process.sequential:
